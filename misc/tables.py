...
class FacultetusActivity(Base):
    ...
    address = Column(VARCHAR(100), comment="Адрес")
    online = Column(
        VARCHAR(1), comment="Предусмотрено онлайн-участие", name="online", quote=True
    )
    external_link = Column(VARCHAR(150), comment="Внешняя ссылка")
    ...
    activity_link = Column(VARCHAR(150))
