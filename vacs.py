...
def main():
    ...
    while True:
        ...
        for vac in response.json()['response']:
            if not session.query(FacultetusVac).filter(
                FacultetusVac.position_id == vac['position_id']
            ).one_or_none():
                vac['spheres'] = transform_list_to_str(vac['spheres']) if vac.get('spheres') else None
                vac['langs'] = transform_list_to_str(vac['langs']) if vac.get('langs') else None
                vac['skills'] = transform_list_to_str(vac['skills']) if vac.get('skills') else None
                vac['tests'] = transform_list_to_str(vac['tests']) if vac.get('tests') else None
                vac['professions'] = transform_list_to_str(vac['professions']) if vac.get('professions') else None
